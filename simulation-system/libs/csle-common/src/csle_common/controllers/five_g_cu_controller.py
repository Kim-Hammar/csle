...
class FiveGCUController:
    ...
    def start_five_g_cu_manager(emulation_env_config: EmulationEnvConfig, ip: str, logger: logging.Logger) -> None:
        ...
        if not_running:
            ...
            # Wait for the 5G CU manager to become ready before returning
            EmulationUtil.wait_for_running_manager(
                status_query=lambda: FiveGCUController.get_five_g_cu_status_by_ip_and_port(
                    ip=ip, port=emulation_env_config.five_g_config.five_g_cu_manager_port, timeout=5),
                ip=ip, manager_name="5G CU manager", logger=logger)
        else:
