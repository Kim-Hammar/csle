...
import './FiveGDUCPUsageChart.css'
import {
  CartesianGrid,
  Label,
  Legend, Line,
...
/**
 * Component containing a plot showing the CPU usage over time
 */
const FiveGDUCPUUsageChart = React.memo((props) => {
    const margin = {
      top: 10,
      right: 30,
      left: 15,
    ...
    if (props.stats !== undefined && props.stats !== null && props.stats.length > 0) {
      const data = props.stats.map((du_metrics, index) => {
        return {
          't': (index + 1),
          'CPU usage %': parseFloat(du_metrics.cpu_usage_percent)
        }
      })
        ...
        <div style={{ height: 300 }}>
          <ResponsiveContainer width="100%" height="100%">
            ...
            >
              ...
              <YAxis type="number">
                <Label angle={270} value="CPU usage (%)" offset={0} position="insideLeft"
                       className="largeFont"
                       dy={50} />
              </YAxis>
              <Tooltip />
...
FiveGDUCPUUsageChart.displayName = 'FiveGDUCPUUsageChart'
FiveGDUCPUUsageChart.propTypes = {}
FiveGDUCPUUsageChart.defaultProps = {}
export default FiveGDUCPUUsageChart
