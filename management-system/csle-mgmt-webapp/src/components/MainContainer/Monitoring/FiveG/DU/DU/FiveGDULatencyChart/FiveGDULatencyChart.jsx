...
import './FiveGDULatencyChart.css'
import {
  CartesianGrid,
  Label,
  Legend, Line,
...
/**
 * Component containing a plot showing the average processing latency over time
 */
const FiveGDULatencyChart = React.memo((props) => {
    const margin = {
      top: 10,
      right: 30,
      left: 15,
    ...
    if (props.stats !== undefined && props.stats.length > 0) {
      const data = props.stats.map((du_metrics, index) => {
        return {
          't': (index + 1),
          'Average': parseFloat(du_metrics.average_latency_us)
        }
      })
...
FiveGDULatencyChart.displayName = 'FiveGDULatencyChart'
FiveGDULatencyChart.propTypes = {}
FiveGDULatencyChart.defaultProps = {}
export default FiveGDULatencyChart
