...
import './FiveGDUAppResourceUsageCPUChart.css'
import {
  CartesianGrid,
  Label,
  Legend, Line,
...
/**
 * Component containing a plot showing the application CPU usage over time for the 5G DU
 */
const FiveGDUAppResourceUsageCPUChart = React.memo((props) => {
    ...
    if (props.stats !== undefined && props.stats !== null && props.stats.length > 0) {
      const data = props.stats.map((app_resource_usage_metrics, index) => {
        return {
          't': (index + 1),
          'CPU usage %': parseFloat(app_resource_usage_metrics.cpu_usage_percent)
        }
      })
