...
import './FiveGDUCellLatencyChart.css'
import {
  CartesianGrid,
  Label,
  Legend, Line,
...
/**
 * Component containing a plot showing the latency of the cell in processing/scheduling of user traffic
 */
const FiveGDUCellLatencyChart = React.memo((props) => {
    const margin = {
      top: 10,
      right: 30,
      left: 15,
    ...
    if (props.stats !== undefined && props.stats !== null && props.stats.length > 0) {
      const data = props.stats.map((five_g_du_cell_metrics, index) => {
        return {
          't': (index + 1),
          'Cell scheduling processing latency (us)': parseFloat(five_g_du_cell_metrics.average_latency)
        }
      })
          ...
          <ResponsiveContainer width="100%" height="100%">
            ...
            >
              ...
              <Line isAnimationActive={props.animation} animation={props.animation} type="monotone"
                    dataKey="Cell scheduling processing latency (us)"
...
FiveGDUCellLatencyChart.displayName = 'FiveGDUCellLatencyChart'
FiveGDUCellLatencyChart.propTypes = {}
FiveGDUCellLatencyChart.defaultProps = {}
export default FiveGDUCellLatencyChart
