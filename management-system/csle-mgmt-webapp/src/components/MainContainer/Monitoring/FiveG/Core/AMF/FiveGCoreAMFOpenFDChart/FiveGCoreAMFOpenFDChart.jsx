...
import './FiveGCoreAMFOpenFDChart.css'
import {
  CartesianGrid,
  Label,
  Legend, Line,
...
/**
 * Component containing a plot showing the open file descriptors over time for the AMF service in the 5G Core
 */
const FiveGCoreAMFOpenFDChart = React.memo((props) => {
    const margin = {
      top: 10,
      right: 30,
      left: 15,
    ...
    if (props.stats !== undefined && props.stats !== null && props.stats.length > 0) {
      const data = props.stats.map((amf_metrics, index) => {
        return {
          't': (index + 1),
          'Number of open file descriptors': parseInt(amf_metrics.process_open_fds)
        }
      })
...
FiveGCoreAMFOpenFDChart.displayName = 'FiveGCoreAMFCPUUsageChart'
FiveGCoreAMFOpenFDChart.propTypes = {}
FiveGCoreAMFOpenFDChart.defaultProps = {}
export default FiveGCoreAMFOpenFDChart
