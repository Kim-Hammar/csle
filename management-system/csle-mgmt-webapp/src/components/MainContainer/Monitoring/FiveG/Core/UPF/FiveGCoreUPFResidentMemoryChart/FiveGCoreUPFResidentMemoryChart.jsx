...
import './FiveGCoreUPFResidentMemoryChart.css'
import {
  CartesianGrid,
  Label,
  Legend, Line,
...
/**
 * Component containing a plot showing the resident memory usage over time for the UPF service in the 5G Core
 */
const FiveGCoreUPFResidentMemoryChart = React.memo((props) => {
    const margin = {
      top: 10,
      right: 30,
      left: 15,
    ...
    if (props.stats !== undefined && props.stats.length > 0) {
      const data = props.stats.map((upf_metrics, index) => {
        return {
          't': (index + 1),
          'Resident memory size (bytes)': parseFloat(upf_metrics.process_resident_memory_bytes)
        }
      })
...
FiveGCoreUPFResidentMemoryChart.displayName = 'FiveGCoreUPFResidentMemoryChart'
FiveGCoreUPFResidentMemoryChart.propTypes = {}
FiveGCoreUPFResidentMemoryChart.defaultProps = {}
export default FiveGCoreUPFResidentMemoryChart
