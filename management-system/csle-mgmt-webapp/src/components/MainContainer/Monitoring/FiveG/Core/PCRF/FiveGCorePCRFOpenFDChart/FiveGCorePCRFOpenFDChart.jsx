...
import './FiveGCorePCRFOpenFDChart.css'
import {
  CartesianGrid,
  Label,
  Legend, Line,
...
/**
 * Component containing a plot showing the open file descriptors over time for the PCRF service in the 5G Core
 */
const FiveGCorePCRFOpenFDChart = React.memo((props) => {
    const margin = {
      top: 10,
      right: 30,
      left: 15,
    ...
    if (props.stats !== undefined && props.stats !== null && props.stats.length > 0) {
      const data = props.stats.map((pcrf_metrics, index) => {
        return {
          't': (index + 1),
          'Number of open file descriptors': parseInt(pcrf_metrics.process_open_fds)
        }
      })
...
FiveGCorePCRFOpenFDChart.displayName = 'FiveGCorePCRFCPUUsageChart'
FiveGCorePCRFOpenFDChart.propTypes = {}
FiveGCorePCRFOpenFDChart.defaultProps = {}
export default FiveGCorePCRFOpenFDChart
