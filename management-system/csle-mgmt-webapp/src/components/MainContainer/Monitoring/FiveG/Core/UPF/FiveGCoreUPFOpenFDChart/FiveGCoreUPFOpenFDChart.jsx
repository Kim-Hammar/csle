...
import './FiveGCoreUPFOpenFDChart.css'
import {
  CartesianGrid,
  Label,
  Legend, Line,
...
/**
 * Component containing a plot showing the open file descriptors over time for the UPF service in the 5G Core
 */
const FiveGCoreUPFOpenFDChart = React.memo((props) => {
    const margin = {
      top: 10,
      right: 30,
      left: 15,
    ...
    if (props.stats !== undefined && props.stats !== null && props.stats.length > 0) {
      const data = props.stats.map((upf_metrics, index) => {
        return {
          't': (index + 1),
          'Number of open file descriptors': parseInt(upf_metrics.process_open_fds)
        }
      })
...
FiveGCoreUPFOpenFDChart.displayName = 'FiveGCoreUPFCPUUsageChart'
FiveGCoreUPFOpenFDChart.propTypes = {}
FiveGCoreUPFOpenFDChart.defaultProps = {}
export default FiveGCoreUPFOpenFDChart
