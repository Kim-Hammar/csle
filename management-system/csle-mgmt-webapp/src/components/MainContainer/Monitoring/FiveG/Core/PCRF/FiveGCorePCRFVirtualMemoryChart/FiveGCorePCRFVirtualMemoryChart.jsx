...
import './FiveGCorePCRFVirtualMemoryChart.css'
import {
  CartesianGrid,
  Label,
  Legend, Line,
...
/**
 * Component containing a plot showing the virtual memory usage over time for the PCRF service in the 5G Core
 */
const FiveGCorePCRFVirtualMemoryChart = React.memo((props) => {
    const margin = {
      top: 10,
      right: 30,
      left: 15,
    ...
    if (props.stats !== undefined && props.stats.length > 0) {
      const data = props.stats.map((pcrf_metrics, index) => {
        return {
          't': (index + 1),
          'Virtual memory size (bytes)': parseFloat(pcrf_metrics.process_virtual_memory_bytes)
        }
      })
...
FiveGCorePCRFVirtualMemoryChart.displayName = 'FiveGCorePCRFVirtualMemoryChart'
FiveGCorePCRFVirtualMemoryChart.propTypes = {}
FiveGCorePCRFVirtualMemoryChart.defaultProps = {}
export default FiveGCorePCRFVirtualMemoryChart
