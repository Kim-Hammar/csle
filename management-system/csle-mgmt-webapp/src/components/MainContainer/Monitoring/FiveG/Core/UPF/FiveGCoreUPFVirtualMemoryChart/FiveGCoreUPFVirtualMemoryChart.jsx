...
import './FiveGCoreUPFVirtualMemoryChart.css'
import {
  CartesianGrid,
  Label,
  Legend, Line,
...
/**
 * Component containing a plot showing the virtual memory usage over time for the UPF service in the 5G Core
 */
const FiveGCoreUPFVirtualMemoryChart = React.memo((props) => {
    const margin = {
      top: 10,
      right: 30,
      left: 15,
    ...
    if (props.stats !== undefined && props.stats.length > 0) {
      const data = props.stats.map((upf_metrics, index) => {
        return {
          't': (index + 1),
          'Virtual memory size (bytes)': parseFloat(upf_metrics.process_virtual_memory_bytes)
        }
      })
...
FiveGCoreUPFVirtualMemoryChart.displayName = 'FiveGCoreUPFVirtualMemoryChart'
FiveGCoreUPFVirtualMemoryChart.propTypes = {}
FiveGCoreUPFVirtualMemoryChart.defaultProps = {}
export default FiveGCoreUPFVirtualMemoryChart
