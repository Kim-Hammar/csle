...
import './FiveGCoreMMEResidentMemoryChart.css'
import {
  CartesianGrid,
  Label,
  Legend, Line,
...
/**
 * Component containing a plot showing the resident memory usage over time for the MME service in the 5G Core
 */
const FiveGCoreMMEResidentMemoryChart = React.memo((props) => {
    const margin = {
      top: 10,
      right: 30,
      left: 15,
    ...
    if (props.stats !== undefined && props.stats.length > 0) {
      const data = props.stats.map((mme_metrics, index) => {
        return {
          't': (index + 1),
          'Resident memory size (bytes)': parseFloat(mme_metrics.process_resident_memory_bytes)
        }
      })
...
FiveGCoreMMEResidentMemoryChart.displayName = 'FiveGCoreMMEResidentMemoryChart'
FiveGCoreMMEResidentMemoryChart.propTypes = {}
FiveGCoreMMEResidentMemoryChart.defaultProps = {}
export default FiveGCoreMMEResidentMemoryChart
