...
import './FiveGDULowCPUUsageChart.css'
import {
  CartesianGrid,
  Label,
  Legend, Line,
...
/**
 * Component containing a plot showing the average CPU usage % over time
 */
const FiveGDULowCPUUsageChart = React.memo((props) => {
    const margin = {
      top: 10,
      right: 30,
      left: 15,
    ...
    if (props.stats !== undefined && props.stats.length > 0) {
      const data = props.stats.map((five_g_du_low_metrics, index) => {
        return {
          't': (index + 1),
          'DL': parseFloat(five_g_du_low_metrics.dl_cpu_usage_percent),
          'UL': parseFloat(five_g_du_low_metrics.ul_cpu_usage_percent)
        }
      })
...
FiveGDULowCPUUsageChart.displayName = 'FiveGDULowCPUUsageChart'
FiveGDULowCPUUsageChart.propTypes = {}
FiveGDULowCPUUsageChart.defaultProps = {}
export default FiveGDULowCPUUsageChart
