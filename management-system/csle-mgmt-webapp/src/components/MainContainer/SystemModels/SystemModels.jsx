...
const SystemModels = (props) => {
    ...
    fetch(
      ...
      .then(response => {
        ...
        if (response !== null && response !== undefined && !(Object.keys(response).length === 0)) {
          ...
          const metricOptions = getMetricForSelectedConditional(conditionalOptions, [conditionalOptions[0]]).map((metricName) => {
            return {
              value: metricName,
              label: metricName
            }
      ...
      .then(response => {
        ...
        if (response !== null && response !== undefined && !(Object.keys(response).length === 0)) {
          ...
          const metricOptions = getMetricForSelectedConditional(conditionalOptions,
            [conditionalOptions[0]]).map((metricName) => {
            return {
              value: metricName,
              label: metricName
            }
  ...
  const fetchSystemModelsIds = useCallback(() => {
    fetch(
      ...
      .then(response => {
        ...
        const modelIds = response.map((id_obj) => {
          return {
  ...
  const MixtureCovariancesOrEmpty = (props) => {
    if (props.loadingSelectedSystemModel ||
      ...
      return props.selectedConditionals.map((conditional) => {
                      ...
                      <tr>
                        ...
                        <td>
                          <Button variant="link" className="dataDownloadLink"
                                  onClick={() => {
                                    const blob = new Blob(
                                      [JSON.stringify(props.selectedSystemModel)],
